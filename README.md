...
Maintain a JFrog access token in [AWS Secrets Manager](https://aws.amazon.com/secrets-manager/) with automatic rotation, so [AWS ECS](https://aws.amazon.com/ecs/) (and other consumers) can pull private registry images using short-lived tokens—without storing long-lived JFrog credentials in the rotation function. Rotation is controlled by the Lambda execution IAM role and JFrog AWS IAM role tagging.
